...
class HomeFragment : Fragment() {
    ...
    private val adapter = YoutubeVideoAdapter { video ->
        extractAndPlay(video.url)
    }
    ...
    private fun loadFeed() {
        ...
        lifecycleScope.launch {
            try {
                val (videos, _) = YouTubeApiClient.getSubscriptionFeed(requireContext())
                adapter.submitList(videos)
                b.tvEmpty.visibility = if (videos.isEmpty()) View.VISIBLE else View.GONE
            } catch (e: Exception) {
                b.tvEmpty.text = "Failed to load: ${e.message}"
